...
tab_final = [[1,2,3,4],[12,13,14,5],[11,0,15,6],[10,9,8,7]]
final = hash(tuple(tuple(i[:]) for i in tab_final))

class no:
    tab = [[0 for x in range(4)] for x in range(4)]
    key = 0
    hlin = 0
    pai = None
    g = 0
    ...
    def __init__(self, tabuleiro, pai):
        self.tab = tabuleiro
        self.key = hash(tuple(tuple(i[:]) for i in self.tab))
        self.hlin = h3(tabuleiro)
        if pai is not None:
            self.pai = pai
            self.g = pai.g+1
        else: self.g = 0
        self.f = self.g + self.hlin
        self.zl, self.zc = posicao_zero(self.tab)
...
def entrada(matriz):
    ...
    for k in range(4):
        for j in range(4):
            matriz[k][j] = int(string[contador])
            contador += 1
    return matriz
...
def h2(matriz):
    vetor = []
    fora_ordem = 0
    vetor.append(matriz[0][0])
    vetor.append(matriz[0][1])
...
aux = [[0 for x in range(2)] for x in range(16)]
def h3(matriz):
    for k in range(4):
        for j in range(4):
            aux[tab_final[k][j]][0] = k
            aux[tab_final[k][j]][1] = j
    heuristica = 0
    for k in range(4):
        for j in range(4):
            posK = aux[matriz[k][j]][0]
            posJ = aux[matriz[k][j]][1]
            heuristica += math.fabs(k - posK) + math.fabs(j - posJ)
...
##==================A*=====================================
def troca_posicao_zero(linha, coluna, S):
    matriz = [i[:] for i in S.tab]
    matriz[S.zl][S.zc], matriz[linha][coluna] = matriz[linha][coluna], matriz[S.zl][S.zc]
    tabuleiro = no(matriz, S)
...
def gerar_jogadas(S):
    sucessores = []
    if(S.zc > 0):
        sucessores.append(troca_posicao_zero(S.zl, S.zc -1, S))
    if(S.zc < 3):
        sucessores.append(troca_posicao_zero(S.zl, S.zc +1,  S))
    if(S.zl > 0):
        sucessores.append(troca_posicao_zero(S.zl - 1, S.zc, S))
    if(S.zl < 3):
        sucessores.append(troca_posicao_zero(S.zl + 1, S.zc, S))
...
def aestrela(S):
    cj_abertos = SortedList()
    cj_abertos.add(S)
    abertos_dict = {S.key: S.f}
    fechados_dict = {}
    while(cj_abertos):
        menor = cj_abertos.pop(0)
        try:
        ...
        if (menor.key == final):
            return menor.g
        fechados_dict[menor.key] = menor.f
        cj_sucessores = gerar_jogadas(menor)
        for sucessor in cj_sucessores:

            if sucessor.key in abertos_dict:
                if abertos_dict[sucessor.key] > sucessor.f:
                    abertos_dict[sucessor.key] = sucessor.f
                    cj_abertos.discard(sucessor)
                    cj_abertos.add(sucessor)

            elif sucessor.key in fechados_dict:
                if fechados_dict[sucessor.key] > sucessor.f:
                    cj_abertos.add(sucessor)
                    abertos_dict[sucessor.key] = sucessor.f
                    del fechados_dict[sucessor.key]
...
def main():
    ...
    print(aestrela(S))
    #fim = time.time()
    #print ("Tempo gasto: ",fim - ini,"segundos")
